...
try:
    ...
    clickhouse = payload.get("clickhouse", {})
    print(f"data-source mode={mode} clickhouse_enabled={clickhouse.get('enabled')} connected={clickhouse.get('connected')} error={clickhouse.get('error')}")
    if ds_code != 200:
        all_ok = False
