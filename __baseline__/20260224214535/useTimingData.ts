...
export function useTimingData(): UseTimingDataResult {
  ...
  const rows = useMemo(() => {
    ...
    const rows: TimingRow[] = index.driverBundles.map(({ driver, laps, prefixValidCount, prefixPitCount, prefixBestLap, prefixBestS1, prefixBestS2, prefixBestS3 }) => {
      ...
      if (
        completedCount > 0 &&
        !currentInProgressLap &&
        effectiveSessionTime > lastLapEnd + 5 &&
        completedCount < index.leaderMaxLap - 1
      ) {
        status = 'dnf'
      } else if (lapDuration > 120 && lapProgress > 0.8) status = 'pit'

      if (status === 'dns') lapDistance = 0
      if (status === 'dnf') lapDistance = completedCount
    ...
    rows.sort((a, b) => {
      ...
      const leaderElapsedPrefix = index.elapsedByDriverNumber.get(rows[0].driverNumber) ?? []
        const leaderDistance = rows[0].lapDistance
        rows[0].gap = 'LEADER'
        rows[0].interval = '—'

        const elapsedAtLapDistance = (
          laps: LapRow[],
          prefixElapsedTime: number[],
          lapNumber: number,
          progress: number
        ...
        const idx = Math.max(0, Math.min(laps.length - 1, lapNumber - 1))
        const lap = laps[idx]
        const duration = lapDurationSeconds(lap) || Math.max(1, lap.lapEndSeconds - lap.lapStartSeconds)
        const base = idx > 0 ? prefixElapsedTime[Math.min(prefixElapsedTime.length - 1, idx - 1)] ?? 0 : 0
          return base + Math.max(0, Math.min(1, progress)) * duration
        }

        const elapsedForRow = (row: TimingRow) => {
          ...
          const lap = laps[idx]
          const lapStart = Number(lap.lapStartSeconds)
          const durationOverride = Number.isFinite(row.lapTimeRef) ? Math.max(1, row.lapTimeRef) : lapDurationSeconds(lap)
          if (!Number.isFinite(lapStart)) return Number.NaN
          const progress = Math.max(0, Math.min(1, row.lapProgress))
          return lapStart + progress * (durationOverride > 0 ? durationOverride : 1)
        }

        const leaderElapsed = elapsedForRow(rows[0])
        ...
        for (let i = 1; i < rows.length; i += 1) {
          ...
          const driverElapsed = elapsedForRow(rows[i])
          const aheadElapsed = elapsedForRow(rows[i - 1])
          const timeGapToLeader = Number.isFinite(leaderElapsed) && Number.isFinite(driverElapsed)
            ? Math.max(0, driverElapsed - leaderElapsed)
            : Number.NaN
          const intervalToAhead = Number.isFinite(aheadElapsed) && Number.isFinite(driverElapsed)
            ? Math.max(0, driverElapsed - aheadElapsed)
            : Number.NaN
