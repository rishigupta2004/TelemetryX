...
export const PitStrategy = React.memo(function PitStrategy() {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-md border border-border bg-bg-card">
            ...
            {(['position', 'result', 'alpha'] as SortMode[]).map((mode) => (
              <button
                ...
                className={`rounded px-1.5 py-0.5 text-[10px] uppercase tracking-[0.08em] ${
                  sortMode === mode ? 'bg-bg-selected text-text-primary' : 'text-text-muted hover:text-text-primary'
                }`}
              >
      ...
      <div className="flex-1 min-h-0 space-y-1 overflow-y-auto px-3 py-2">
        {driverStints.map(({ driver, position, finalPosition, stints, strategy }) => {
          ...
          return (
            <div
              key={`${driver}-${strategy}`}
              className={`grid h-8 grid-cols-[96px_1fr_50px] items-center gap-2 rounded border border-transparent px-1 transition hover:border-white/10 hover:bg-bg-secondary/60 ${
                pitNow ? 'tx-pit-flash' : ''
              }`}
            >
              ...
              <div className="relative h-6 w-full overflow-visible rounded border border-border/70 bg-bg-secondary/40">
                {stints.map((stint, idx) => {
                  ...
                  const textColor = compound === 'HARD' ? '#111111' : color
                  ...
                  return (
                    ...
                    >
                      {isPitBoundary && (
                        <div
                          className="pointer-events-none absolute -left-[1px] top-[-3px] h-[26px] w-[2px] bg-white/90 shadow-[0_0_10px_rgba(255,255,255,0.55)]"
                      ...
                      <div className="pointer-events-none absolute bottom-[120%] left-1/2 z-20 hidden w-[220px] -translate-x-1/2 rounded border border-border bg-[#07101e]/95 p-1.5 text-[10px] text-text-secondary shadow-[0_12px_24px_rgba(0,0,0,0.4)] group-hover:block">
                        {tooltip}
                      </div>
                    </div>
                  )
                ...
                {undercut && undercut.total > 0 && (
                  <div
                    className={`pointer-events-none absolute right-1 top-1/2 flex -translate-y-1/2 items-center gap-1 rounded px-1.5 py-0.5 text-[9px] font-mono ${
                      undercut.successRate >= 0.5
          ...
          {LEGEND_COMPOUNDS.map((compound) => {
            ...
            return (
              <div key={compound} className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-text-muted">{readableCompound(compound)}</span>
              </div>
            )
          })}
        </div>
