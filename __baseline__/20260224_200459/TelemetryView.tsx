...
const CHARTS: {
  ...
  { key: 'speed', title: 'Speed', h: 150, y: [0, 360], axisLabel: 'Speed (km/h)', tickMode: 'default', tickUnit: 'km/h', markers: true },
  { key: 'throttle', title: 'Throttle', h: 90, y: [0, 105], axisLabel: 'Throttle (%)', tickMode: 'percent' },
  { key: 'brake', title: 'Brake', h: 90, y: [0, 105], axisLabel: 'Brake (%)', tickMode: 'percent' },
  { key: 'gear', title: 'Gear', h: 70, y: [0, 9], axisLabel: 'Gear', tickMode: 'integer', stepped: true },
  { key: 'rpm', title: 'RPM', h: 90, y: [0, 15000], axisLabel: 'Engine Speed (RPM)', tickMode: 'rpm' },
  { key: 'drs', title: 'DRS', h: 50, y: [0, 2], axisLabel: 'DRS State', tickMode: 'binary', stepped: true }
]
      ...
      <div className="glass-panel flex flex-shrink-0 flex-wrap items-center gap-3 rounded-t-2xl border-b-0 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="rounded-lg border border-border/70 bg-[#12284abf] px-2.5 py-1 text-sm text-text-primary"
          >
            {drivers.map((d) => (
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            className={`rounded-lg border px-2.5 py-1 text-xs font-mono ${
              followPlayback
                ? 'border-accent-blue bg-accent-blue/25 text-text-primary'
                : 'border-border bg-[#10223dba] text-text-secondary'
            }`}
          >
          ...
          <button
            ...
            disabled={lapIdx <= 0}
            className="rounded-lg border border-border bg-[#10223dba] px-2 py-1 text-sm text-text-secondary hover:text-text-primary disabled:cursor-not-allowed disabled:opacity-30"
          >
            ◀
          </button>
          ...
          <select
            ...
            className="rounded-lg border border-border bg-[#10223dba] px-2.5 py-1 font-mono text-sm text-text-primary"
          >
            {lapNumbers.map((lap) => (
        ...
        <div className="w-full text-[11px] text-text-muted md:w-auto">
