...
export function RacePaceLiteChart({ series }: { series: RacePaceSeries[] }) {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${view.width} ${view.height}`} className="h-[360px] w-full">
        <rect x="0" y="0" width={view.width} height={view.height} fill="#0a1122" />
        <line x1={view.left} y1={view.top + view.innerH} x2={view.left + view.innerW} y2={view.top + view.innerH} stroke="rgba(132,160,200,0.35)" />
        <line x1={view.left} y1={view.top} x2={view.left} y2={view.top + view.innerH} stroke="rgba(132,160,200,0.35)" />
        {view.paths.map((path) => (
          <path key={path.code} d={path.d} fill="none" stroke={path.color} strokeWidth="2.1" opacity="0.92" vectorEffect="non-scaling-stroke" />
        ))}
        <text x={view.left + 4} y={12} fill="rgba(154,180,217,0.9)" fontSize="10">Lap Time (s)</text>
        <text x={view.left + view.innerW - 40} y={view.top + view.innerH + 18} fill="rgba(154,180,217,0.9)" fontSize="10">Lap</text>
        <text x={view.left + 2} y={view.top + 12} fill="rgba(214,230,255,0.9)" fontSize="10">{formatLapTime(view.yMax)}</text>
        <text x={view.left + 2} y={view.top + view.innerH - 2} fill="rgba(214,230,255,0.9)" fontSize="10">{formatLapTime(view.yMin)}</text>
        <text x={view.left} y={view.top + view.innerH + 16} fill="rgba(214,230,255,0.9)" fontSize="10">{Math.round(view.xMin)}</text>
        <text x={view.left + view.innerW - 18} y={view.top + view.innerH + 16} fill="rgba(214,230,255,0.9)" fontSize="10">{Math.round(view.xMax)}</text>
      </svg>
...
export function SeasonStandingsLiteChart({ drivers, raceCount }: { drivers: StandingsSeries[]; raceCount: number }) {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${view.width} ${view.height}`} className="h-[340px] w-full">
        <rect x="0" y="0" width={view.width} height={view.height} fill="#0a1122" />
        <line x1={view.left} y1={view.top + view.innerH} x2={view.left + view.innerW} y2={view.top + view.innerH} stroke="rgba(132,160,200,0.35)" />
        <line x1={view.left} y1={view.top} x2={view.left} y2={view.top + view.innerH} stroke="rgba(132,160,200,0.35)" />
        {view.paths.map((path) => (
        ...
        <text x={view.left + 4} y={12} fill="rgba(154,180,217,0.9)" fontSize="10">Cumulative Points</text>
        <text x={view.left + view.innerW - 54} y={view.top + view.innerH + 18} fill="rgba(154,180,217,0.9)" fontSize="10">Race Index</text>
      </svg>
...
export function StandingsHeatmapLiteChart({ raceNames, drivers }: { raceNames: string[]; drivers: StandingsHeatmapRow[] }) {
  ...
  const color = (value: number) => {
    ...
    const r = Math.round(26 + t * 70)
    const g = Math.round(41 + t * 110)
    const b = Math.round(75 + t * 170)
    return `rgb(${r}, ${g}, ${b})`
  }

  return (
    <div className="overflow-x-auto rounded border border-white/10 bg-[#0d1528] p-2">
      <div
      ...
      >
        ...
        {raceNames.map((_race, idx) => (
          <div key={`h-head-${idx}`} className="text-center text-[10px] text-text-muted">R{idx + 1}</div>
        ))}

        {drivers.map((driver) => (
          <Fragment key={`h-row-${driver.code}`}>
            <div className="flex items-center gap-1 text-[10px] font-mono text-text-primary">
              <span className="h-2 w-2 rounded-full" style={{ backgroundColor: driver.color }} />
            ...
            {driver.byRace.map((value, idx) => {
              ...
              return (
                <div
                  ...
                  className="flex h-7 items-center justify-center rounded text-[10px] font-mono text-[#e2eeff]"
