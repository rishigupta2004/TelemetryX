  ...
  return (
    <div>
      <svg viewBox={`0 0 ${view.width} ${view.height}`} className="h-[360px] w-full">
        <rect x="0" y="0" width={view.width} height={view.height} fill="#0b0e12" />
        <line x1={view.left} y1={view.top + view.innerH} x2={view.left + view.innerW} y2={view.top + view.innerH} stroke="rgba(185,195,210,0.25)" />
        <line x1={view.left} y1={view.top} x2={view.left} y2={view.top + view.innerH} stroke="rgba(185,195,210,0.25)" />
        {view.paths.map((path) => (
          <path key={path.code} d={path.d} fill="none" stroke={path.color} strokeWidth="2.1" opacity="0.92" vectorEffect="non-scaling-stroke" />
        ))}
        <text x={view.left + 4} y={12} fill="rgba(200,210,224,0.9)" fontSize="10">Lap Time (s)</text>
        <text x={view.left + view.innerW - 40} y={view.top + view.innerH + 18} fill="rgba(200,210,224,0.9)" fontSize="10">Lap</text>
        <text x={view.left + 2} y={view.top + 12} fill="rgba(226,234,244,0.9)" fontSize="10">{formatLapTime(view.yMax)}</text>
        <text x={view.left + 2} y={view.top + view.innerH - 2} fill="rgba(226,234,244,0.9)" fontSize="10">{formatLapTime(view.yMin)}</text>
        <text x={view.left} y={view.top + view.innerH + 16} fill="rgba(226,234,244,0.9)" fontSize="10">{Math.round(view.xMin)}</text>
        <text x={view.left + view.innerW - 18} y={view.top + view.innerH + 16} fill="rgba(226,234,244,0.9)" fontSize="10">{Math.round(view.xMax)}</text>
      </svg>
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${view.width} ${view.height}`} className="h-[340px] w-full">
        ...
        <text x={view.left + 4} y={12} fill="rgba(200,210,224,0.9)" fontSize="10">Cumulative Points</text>
        <text x={view.left + view.innerW - 54} y={view.top + view.innerH + 18} fill="rgba(200,210,224,0.9)" fontSize="10">Race Index</text>
      </svg>
...
export function StandingsHeatmapLiteChart({ raceNames, drivers }: { raceNames: string[]; drivers: StandingsHeatmapRow[] }) {
  ...
  const color = (value: number) => {
    ...
    const r = Math.round(32 + t * 100)
    const g = Math.round(38 + t * 120)
    const b = Math.round(48 + t * 140)
    return `rgb(${r}, ${g}, ${b})`
  }

  return (
    <div className="overflow-x-auto rounded border border-white/10 bg-[#0b0e12] p-2">
      <div
