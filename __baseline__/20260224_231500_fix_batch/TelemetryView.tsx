...
const CHARTS: {
  ...
  { key: 'speed', title: 'Speed', h: 320, axisLabel: 'Speed (km/h)', tickMode: 'default', tickUnit: 'km/h', markers: true },
  { key: 'throttle', title: 'Throttle', h: 220, y: [0, 105], axisLabel: 'Throttle (%)', tickMode: 'percent' },
  { key: 'brake', title: 'Brake', h: 220, y: [0, 105], axisLabel: 'Brake (%)', tickMode: 'percent' },
  { key: 'gear', title: 'Gear', h: 180, y: [0, 9], axisLabel: 'Gear', tickMode: 'integer', stepped: true },
  { key: 'rpm', title: 'RPM', h: 240, axisLabel: 'Engine Speed (RPM)', tickMode: 'rpm' },
  { key: 'drs', title: 'DRS', h: 140, y: [0, 2], axisLabel: 'DRS State', tickMode: 'binary', stepped: true }
]
...
export const TelemetryView = React.memo(function TelemetryView() {
  ...
  const builtCharts = useMemo((): BuiltChart[] => {
    ...
    const nowX = showPlaybackMarker ? distanceAtTime(sampledSessionTime) : 0

    const baseCharts = CHARTS.map((cfg) => {
      const pair = chartData[cfg.key]
      const series = [
        { label: selectedDriver, data: pair.primary, color: '#82cfff', width: 2.6 },
        ...(compareDriver && pair.compare.length
          ? [{ label: compareDriver, data: pair.compare, color: '#a6b0bf', width: 2.1 }]
          : [])
      ]
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden p-5 xl:p-6">
      ...
      <div className="glass-panel flex-1 overflow-y-auto rounded-b-2xl border-t-0 px-3 py-3">
        {builtCharts.length ? (
          builtCharts.map((cfg) => {
            const cursor = cursorByKey[cfg.key]
            const cursorLabel =
              cursor && cursor.x != null
                ? `x=${Math.round(cursor.x)}m · ${cfg.series
                    .map((series, idx) => {
                      const value = cursor.values[idx]
                      if (!Number.isFinite(value)) return `${series.label}: —`
                      return `${series.label}: ${formatMetricValue(value, cfg.yTickMode, cfg.yTickUnit)}`
                    })
                    .join(' | ')}`
                : cfg.subtitle

            return (
              <div key={cfg.key} className="mb-6 last:mb-0 rounded-xl border border-border/70 bg-black/25 p-3">
                <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
                  <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-text-secondary">
                    {cfg.title}
                  </div>
                  {cursorLabel && (
                    <div className="font-mono text-[10px] text-text-muted">{cursorLabel}</div>
                  )}
                </div>
                <UPlotChart
                  title={cfg.title}
                  ...
                  frame={false}
                  showHeader={false}
                  onCursor={handleCursor(String(cfg.key))}
                />
              </div>
            )
          })
