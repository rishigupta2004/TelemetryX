...
export function useTimingData(): UseTimingDataResult {
  ...
  const rows = useMemo(() => {
    ...
    const rows: TimingRow[] = index.driverBundles.map(({ driver, laps, prefixValidCount, prefixPitCount, prefixBestLap, prefixBestS1, prefixBestS2, prefixBestS3 }) => {
      ...
      const derivedLapNumber =
        carPos && Number.isFinite(carPos.currentLap) && carPos.currentLap > 0
          ? carPos.currentLap
          : currentLapData.lapNumber
      if (carPos) {
      ...
      if (completedCount > 0 && !currentInProgressLap) {
        ...
        const stallThreshold = Math.min(20, Math.max(8, lastLapDuration * 0.2))
    ...
    if (rows.length > 0 && (rows[0].status === 'racing' || rows[0].status === 'pit')) {
      ...
      for (let i = 1; i < rows.length; i += 1) {
        ...
        const timeGapToLeader =
          Number.isFinite(leaderLapSeconds) && leaderLapSeconds > 0
            ? Math.max(0, lapDeltaToLeader) * leaderLapSeconds
            : Number.NaN
        const intervalToAhead =
          Number.isFinite(aheadLapSeconds) && aheadLapSeconds > 0
            ? Math.max(0, lapDeltaToAhead) * aheadLapSeconds
            : Number.NaN
