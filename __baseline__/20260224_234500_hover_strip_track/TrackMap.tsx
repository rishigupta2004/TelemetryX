...
export function TrackMap({ compact = false, mode = 'full' }: TrackMapProps) {
  ...
  const resolvedCars = useMemo(() => {
    ...
    return displayCars.map((car) => {
      ...
      if (Number.isFinite(car.x) && Number.isFinite(car.y)) {
        const margin = 60
        const inBounds =
          car.x >= trackData.bounds.minX - margin &&
          car.x <= trackData.bounds.maxX + margin &&
          car.y >= trackData.bounds.minY - margin &&
          car.y <= trackData.bounds.maxY + margin
        if (inBounds) {
          pos = { x: car.x as number, y: car.y as number }
        }
      }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Track surface
      drawPath(trackData.points, '#0f1217', 30)
      drawPath(trackData.points, 'rgba(190,200,214,0.42)', 22)
      drawPath(trackData.points, '#1c222c', 18)
      drawPath(trackData.points, 'rgba(120,130,145,0.45)', 3.5, [18, 22])
  ...
  return (
    <div
      ...
      style={{ background: '#0b0f14', perspective: '1200px', perspectiveOrigin: '50% 36%' }}
    >
      {!isMinimap && (
