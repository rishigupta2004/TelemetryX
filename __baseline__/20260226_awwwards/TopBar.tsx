...
export default React.memo(function TopBar({
  ...
  const {
    ...
    failedEndpoints: featureFailures
  } = useFeaturesStore((s) => ({
    lap: s.lap,
    tyre: s.tyre,
    ...
    position: s.position,
    raceContext: s.raceContext,
    traffic: s.traffic,
    overtakes: s.overtakes,
    points: s.points,
    loading: s.loading,
    failedEndpoints: s.failedEndpoints
  }))

  const { clustering, strategyRecs, loading: mlLoading, failedEndpoints: mlFailures } = useMLStore((s) => ({
    clustering: s.clustering,
    strategyRecs: s.strategyRecs,
    loading: s.loading,
    failedEndpoints: s.failedEndpoints
  }))
  ...
  const readiness = useMemo(() => {
    const entries = [
      { key: 'lap', label: 'LAP', ready: !!lap },
      { key: 'tyre', label: 'TYR', ready: !!tyre },
      { key: 'telemetry', label: 'TEL', ready: !!telemetry },
      { key: 'comparison', label: 'CMP', ready: !!comparison },
      { key: 'position', label: 'POS', ready: !!position },
      { key: 'race-context', label: 'CTX', ready: !!raceContext },
      { key: 'traffic', label: 'TRF', ready: !!traffic },
      { key: 'overtakes', label: 'OVR', ready: !!overtakes },
      { key: 'points', label: 'PTS', ready: !!points },
      { key: 'clustering', label: 'CLU', ready: !!clustering },
      { key: 'strategy-recs', label: 'STR', ready: !!strategyRecs }
    ]
    return entries.map((entry) => {
      const failed = featureFailures.includes(entry.key) || mlFailures.includes(entry.key)
      const loading = (featuresLoading || mlLoading) && !entry.ready && !failed
      return { ...entry, failed, loading }
    })
  }, [
    lap,
    tyre,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 xl:gap-3" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        ...
        />
        <div
          className="text-base font-bold tracking-[-0.02em] text-white sm:text-lg"
          style={{ fontFamily: "var(--font-display)" }}
        >
          TELEMETRY<span style={{ color: '#e10600' }}>X</span>
        </div>
        <div
      ...
      <div className="mx-3 min-w-0 flex-1 sm:mx-5" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        ...
        {chips.length > 0 && (
          <div className="mt-1 hidden items-center gap-1.5 xl:flex">
            ...
            <div className="flex items-center gap-1.5 rounded-full border border-border/50 bg-bg-card/80 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-text-muted">
              <span className="text-[9px] text-text-muted">Features</span>
              <div className="flex items-center gap-1">
                {readiness.map((entry) => {
                  const color = entry.failed ? '#ff5f5f' : entry.ready ? '#35e080' : entry.loading ? '#ffd600' : '#555560'
                  return (
                    <div
                      key={entry.key}
                      className="flex h-3.5 w-3.5 items-center justify-center rounded-[4px] text-[7px] font-bold text-black/80"
                      title={`${entry.label} ${entry.ready ? 'ready' : entry.failed ? 'failed' : entry.loading ? 'loading' : 'idle'}`}
                      style={{
                        backgroundColor: color,
                        boxShadow: entry.ready ? '0 0 6px rgba(53,224,128,0.5)' : 'none'
                      }}
                    >
                      {entry.label[0]}
                    </div>
