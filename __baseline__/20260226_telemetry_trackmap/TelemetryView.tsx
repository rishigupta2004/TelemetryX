...
const HALF_WINDOW = 60 // ±60s of data centered on current time
const INNER_BUFFER = 30 // only refetch when we exit this inner zone
...
export const TelemetryView = React.memo(function TelemetryView({ active = true }: { active?: boolean }) {
  ...
  const [selectedChannel, setSelectedChannel] = useState<ChannelKey>('speed')
  ...
  // 60fps cursor fraction — updated via rAF subscription to playbackStore, no setState
  const cursorFractionRef = useRef(0)
  const [cursorFraction, setCursorFraction] = useState<number | undefined>(undefined)
  ...
  // Windowed fetch: ±60s centered on current time, only refetch when outside 30s inner buffer
  const fetchWindow = useMemo(() => {
    if (!driverLaps.length) return null
    // When not following playback (manual lap selection), always fetch the selected lap window
    if (!followPlayback && lapTimeWindow) {
    ...
    // If we're still inside the inner buffer of the current window, keep it
    if (cur && t >= cur.t0 + INNER_BUFFER && t <= cur.t1 - INNER_BUFFER) {
      return cur
    }
    // Compute a new window centered on current time, but clamp to lap boundaries if available
    const lapStart = driverLaps[0].lapStartSeconds
    const lapEnd = driverLaps[driverLaps.length - 1].lapEndSeconds
    const t0 = Math.max(0, Math.floor(Math.max(lapStart - 5, t - HALF_WINDOW)))
    const t1 = Math.ceil(Math.min(lapEnd + 5, t + HALF_WINDOW))
    const newWindow = { t0, t1 }
    windowBoundsRef.current = newWindow
    return newWindow
  }, [driverLaps, sampledSessionTime, followPlayback, lapTimeWindow])
  ...
  const builtCharts = useMemo((): BuiltChart[] => {
    ...
    const mappedCharts = CHANNELS.map((cfg) => {
      ...
      const hasData = pair.primary.some((v) => v !== 0 && v != null)
        ...
        <div className="flex items-center gap-1 border-b border-border/40 pb-2">
          ...
          {compareDriver && builtCharts.some((c) => c.key === 'deltaSpeed') && (
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-amber-400 hover:bg-bg-hover`}
            >
              Δ Speed
            </button>
          )}
            ...
            <div className="flex h-full flex-col">
              ...
              <div ref={chartContainerRef} className="relative min-h-0 flex-1 px-2 pb-2">
                <UPlotChart
                  ...
                  playbackCursorFraction={cursorFraction}
                  onSeek={handleSeek}
                />
              </div>
