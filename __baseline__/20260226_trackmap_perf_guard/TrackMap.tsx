...
export function TrackMap({ compact = false, mode = 'full' }: TrackMapProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // === TRACK SURFACE — thick gray road with lighter shoulders ===
      // Lighter shoulders (outer edge)
      drawPath(trackData.points, '#6a6a6a', 30)
      // Road surface (gray asphalt)
      drawPath(trackData.points, '#3f3f3f', 22)
      // Inner road detail (slightly lighter center)
      drawPath(trackData.points, '#4a4a4a', 16)
      // Center line dashes
      drawPath(trackData.points, 'rgba(255,255,255,0.12)', 1.5, [14, 18])

      // === FLAG OVERLAYS ===
      if (currentFlags.isRedFlag) {
        drawPath(trackData.points, 'rgba(255, 30, 30, 0.5)', 24, [], 'rgba(255, 0, 0, 0.8)', 16)
      } else if (currentFlags.isSafetyCar) {
        drawPath(trackData.points, 'rgba(255, 165, 0, 0.45)', 24, [], 'rgba(255, 165, 0, 0.8)', 14)
      } else if (currentFlags.isVSC) {
        drawPath(trackData.points, 'rgba(255, 220, 0, 0.4)', 24, [], 'rgba(255, 220, 0, 0.8)', 12)
      }
          ...
          if (sectorFlag === 'YELLOW' || sectorFlag === 'DOUBLE YELLOW') {
            ...
            if (sectorPts.length >= 2) {
              const yellowColor = sectorFlag === 'DOUBLE YELLOW'
                ? 'rgba(255, 200, 0, 0.55)'
                : 'rgba(255, 220, 0, 0.40)'
              drawPath(sectorPts, yellowColor, 22, [], yellowColor, 12)
            }
          }
        }
      }

      // === PIT LANE — dark road with amber dashes ===
      if (trackData.pitLanePoints && trackData.pitLanePoints.length > 0) {
        drawPath(trackData.pitLanePoints, '#2a2a2a', 10)
        drawPath(trackData.pitLanePoints, 'rgba(255,180,40,0.7)', 3, [8, 8])
      }

      // === DRS ZONES — orange/amber like F1 TV reference ===
      trackData.drsPolylines.forEach(drs => {
        if (!drs.segment || drs.segment.length < 2) return
        drawPath(drs.segment, 'rgba(255, 160, 40, 0.45)', 22)
        drawPath(drs.segment, 'rgba(255, 180, 60, 0.9)', 6)
        if (drs.midPoint) {
          drawLabelPill(drs.midPoint.x, drs.midPoint.y - 20, `DRS`, 'rgba(255,180,40,0.95)', 'rgba(40,25,0,0.85)')
        }
      })
      ...
      trackData.sectorMarkers.forEach(sector => {
        ...
        drawLabelPill(p.x + nx * 2.2, p.y + ny * 2.2, sector.label, sector.color, 'rgba(0,0,0,0.85)')
      })
      ...
      if (trackData.pitEntryIdx != null && trackData.points[trackData.pitEntryIdx]) {
        ...
        ctx.lineWidth = 2
        ctx.stroke()
        drawLabelPill(p.x + 16, p.y, 'PIT IN', 'rgba(255,180,40,0.95)', 'rgba(35,20,0,0.9)')
      }
      if (trackData.pitExitIdx != null && trackData.points[trackData.pitExitIdx]) {
        ...
        ctx.lineWidth = 2
        ctx.stroke()
        drawLabelPill(p.x + 16, p.y, 'PIT OUT', 'rgba(255,180,40,0.95)', 'rgba(35,20,0,0.9)')
      }
      if (trackData.pitLanePolylinePoints && trackData.pitEntryIdx != null && trackData.pitExitIdx != null) {
        ...
        drawLabelPill(midPit.x, midPit.y + 22, 'PIT LANE', 'rgba(255,180,40,0.9)', 'rgba(35,20,0,0.9)')
      }
