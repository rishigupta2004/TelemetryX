...
DATA_ROOT = Path(os.getenv("TELEMETRYX_DATA_ROOT", str(_default_data_root()))).expanduser().resolve()
...
TRACK_GEOMETRY_DIR = Path(
    os.getenv("TELEMETRYX_TRACK_GEOMETRY_DIR", str(DATA_ROOT / "track_geometry"))
).expanduser().resolve()
...
MODELS_DIR = Path(os.getenv("TELEMETRYX_MODELS_DIR", str(REPO_ROOT / "scripts" / "models"))).expanduser().resolve()
