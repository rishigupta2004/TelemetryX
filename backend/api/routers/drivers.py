...
def _fetchall(sql: str, params: List[Any]) -> List[Any]:
    ...
    try:
        ...
        pass  # Connection is pooled, don't close
...
async def get_drivers(
    ...
    if session_type:
        silver_path = os.path.join(
            year_path, race_dir, normalize_session_code(session_type)
        )
    else:
    ...
    try:
        ...
        name_expr = (
            ...
            if has_driver_name
            else "CAST(driver_number AS VARCHAR)"
        )
...
async def get_driver(
    year: int, round: str, driver_id: str, session_type: Optional[str] = None
) -> Dict[str, Any]:
    race_name = round.replace("-", " ")
    year_path = os.path.join(SILVER_DIR, str(year))
    race_dir = resolve_dir(year_path, race_name)
