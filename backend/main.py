...
# Optional auth gate for production. In dev, leave this off to keep the API easy to iterate on.
_REQUIRE_AUTH = os.getenv("TELEMETRYX_REQUIRE_AUTH", "0") == "1"
try:
...
async def request_timing_middleware(request: Request, call_next):
    ...
    try:
        ...
        payload_bytes = 0
    try:
        metrics.record_request(request.url.path, duration_ms, payload_bytes=payload_bytes)
    except Exception:
        pass
...
app.include_router(seasons.router, prefix="/api/v1", tags=["Seasons"], dependencies=_AUTH_DEPS)
app.include_router(races.router, prefix="/api/v1", tags=["Races"], dependencies=_AUTH_DEPS)
app.include_router(sessions.router, prefix="/api/v1", tags=["Sessions"], dependencies=_AUTH_DEPS)
app.include_router(drivers.router, prefix="/api/v1", tags=["Drivers"], dependencies=_AUTH_DEPS)
app.include_router(laps.router, prefix="/api/v1", tags=["Laps"], dependencies=_AUTH_DEPS)
app.include_router(telemetry.router, prefix="/api/v1", tags=["Telemetry"], dependencies=_AUTH_DEPS)
app.include_router(positions.router, prefix="/api/v1", tags=["Positions"], dependencies=_AUTH_DEPS)
app.include_router(streams.router, prefix="/api/v1", tags=["Streams"], dependencies=_AUTH_DEPS)
