...
function eventTag(m: RaceControlMessage): { text: string; cls: string } | null {
  ...
  const base = 'px-1 py-[1px] border text-[9px] font-bold uppercase tracking-widest whitespace-nowrap'
...
const stateBadge = (
  ...
  const base = 'px-1.5 py-[2px] border text-[10px] font-bold uppercase tracking-widest'
...
const RaceControlMessageRow = memo(function RaceControlMessageRow({
  ...
  return (
    <div
      ref={ref}
      className={`relative border border-border-micro p-2 transition-colors ${accent.bg}`}
    >
      {/* Left accent bar */}
      <div className={`absolute left-0 top-0 bottom-0 w-[2px] ${accent.bar}`} />

      <div className="mb-1 flex flex-wrap items-center gap-2 pl-2">
        <span className="font-mono text-[10px] text-fg-secondary tracking-widest">{m.time}</span>
        {m.lap != null && (
          <span className="font-mono text-[10px] text-fg-muted tracking-widest">LAP {m.lap}</span>
        )}
        {tag && (
        ...
        {m.racingNumber != null && (
          <span className="font-mono text-[10px] text-fg-primary font-bold">CAR {m.racingNumber}</span>
        )}
        {i === 0 && (
          <span className="px-1 py-[1px] border border-red-core text-red-core bg-red-ghost text-[9px] font-bold uppercase tracking-widest">
            LATEST
          </span>
        )}
      </div>
...
export const RaceControlFeed = memo(function RaceControlFeed() {
  ...
  if (!visible.length) {
    return (
      <div className="flex h-full items-center justify-center p-3 text-center bg-transparent">
        <div>
          <div className="text-[10px] font-bold text-fg-secondary tracking-widest uppercase pb-1" style={{ fontFamily: 'var(--font-heading)' }}>RACE CONTROL</div>
          <div className="text-[10px] font-mono text-fg-muted tracking-widest uppercase">
  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-hidden">
      {/* Header */}
      <div className="flex flex-col gap-2 border-b border-border-hard pb-2">
        <div className="flex justify-between items-center">
          <div className="text-[11px] font-bold uppercase tracking-widest text-fg-secondary" style={{ fontFamily: 'var(--font-heading)' }}>
