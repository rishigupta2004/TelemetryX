  ...
  return (
    <div ref={controlsRef} className="flex flex-wrap items-center gap-3">
      <button
        ...
        className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[11px] font-mono uppercase tracking-wider transition-all ${
          followPlayback
      ...
      <div className="flex items-center rounded-lg border border-border-hard bg-bg-inset overflow-hidden">
        <button
          type="button"
          onClick={() => {
            if (lapIdx > 0) {
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none bg-transparent px-4 py-1.5 pr-8 font-mono text-[12px] text-fg-primary outline-none cursor-pointer"
          >
            {lapNumbers.map((lap) => (
      ...
      {lapTimeWindow && (
        <div className="flex items-center gap-2 rounded-lg bg-bg-inset px-3 py-1.5 border border-border-micro">
          <span className="text-[9px] text-fg-muted uppercase tracking-wider">Lap Time</span>
          <span className="font-mono text-[13px] text-fg-primary font-semibold">{lapTimeWindow.duration.toFixed(3)}s</span>
        </div>
      )}

      <div className="ml-auto flex items-center gap-3 text-[10px] font-mono text-fg-muted">
        ...
        <span className="flex items-center gap-1.5">
          <span className="text-[9px] uppercase tracking-wider">Window</span>
          <span className="text-fg-secondary">{telemetryWindowStart.toFixed(0)}-{telemetryWindowEnd.toFixed(0)}s</span>
        </span>
