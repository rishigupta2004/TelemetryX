  ...
  return (
    <div className="flex h-full flex-col rounded-md border border-border bg-bg-card">
      <div className="border-b border-border px-3 py-2">
        <div className="text-xs uppercase tracking-[0.18em] text-text-secondary">Undercut Predictor</div>
        <div className="mt-1 font-mono text-[11px] text-text-muted">
          {primaryDriver ? `Primary ${primaryDriver}` : 'No primary driver'}
          {compareDriver ? ` | Compare ${compareDriver}` : ''}
        </div>
      </div>
      ...
      <div className="mt-auto border-t border-border p-3">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <button
            ...
            className="rounded border border-border bg-bg-secondary px-3 py-1.5 text-sm text-text-primary hover:bg-bg-hover disabled:opacity-50"
          >
          ...
          <button
            ...
            className="rounded border border-border bg-bg-card px-3 py-1.5 text-sm text-text-secondary hover:bg-bg-hover disabled:opacity-50"
          >
        ...
        {missingFields.length > 0 && (
          <div className="mb-2 rounded border border-amber-600/50 bg-amber-500/10 p-2 text-xs text-amber-200">
            Waiting for data fields: {missingFields.join(', ')}
          </div>
        )}
        ...
        {result && (
          <div className="rounded border border-border bg-bg-secondary p-2 text-xs">
            <div className="font-mono text-sm text-text-primary">
              {result.prediction} | {(result.success_probability * 100).toFixed(1)}%
            </div>
