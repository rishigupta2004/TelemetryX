...
export const TelemetryHeader = React.memo(function TelemetryHeader({
  ...
  return (
    <div
      ref={headerRef}
      className="flex flex-shrink-0 flex-col gap-2 border-b border-border-soft bg-bg-surface px-4 py-2.5"
    >
      <div className="flex flex-wrap items-center gap-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none rounded-md border border-border-hard bg-bg-inset px-3 py-1.5 pr-7 text-[11px] font-mono text-fg-primary outline-none transition-all hover:border-blue-sel/50 focus:border-blue-sel focus:ring-1 focus:ring-blue-sel/30"
              style={{ minWidth: 148 }}
            >
              {drivers.map((d) => (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none rounded-md border border-border-hard bg-bg-inset px-3 py-1.5 pr-7 text-[11px] font-mono text-fg-primary outline-none transition-all hover:border-purple-sb/50 focus:border-purple-sb focus:ring-1 focus:ring-purple-sb/30"
            >
        ...
        <div className="flex items-center gap-1.5">
          {selectedDriverObj && (
            <div
              className="flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[10px] font-semibold border transition-all"
              style={{
      ...
      <div className="flex items-center gap-2">
        <span className="text-[9px] uppercase tracking-[0.16em] text-fg-muted">Channels</span>
        <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto rounded-md border border-border-hard bg-bg-inset p-1">
          {CHANNELS.slice(0, 6).map((channel) => (
            <button
              key={channel.key}
              onClick={() => onToggleChannel(channel.key as ChannelKey)}
              className={`rounded px-2 py-[5px] text-[10px] font-mono uppercase tracking-[0.12em] transition-all ${
