...
import type { DriverSummaryResponse, LapRow, TyreStint, UndercutPredictRequest, UndercutPredictResponse } from '../types'
...
const RESULT_LABELS: Record<string, string> = {
  FAILURE: 'UNDERCUT NOT RECOMMENDED',
  SUCCESS: 'UNDERCUT RECOMMENDED',
...
function normalizeCompound(compound: string | null | undefined): string | null {
  const raw = String(compound || '').toUpperCase()
  if (!raw) return null
  if (raw.startsWith('S')) return 'SOFT'
  if (raw.startsWith('M')) return 'MEDIUM'
  if (raw.startsWith('H')) return 'HARD'
  if (raw.startsWith('I')) return 'INTERMEDIATE'
  if (raw.startsWith('W')) return 'WET'
  return raw
}

function deriveInputsFromSession(
  laps: LapRow[],
  tyreStints: TyreStint[] | null,
  driverNumber: number | null,
  raceName: string | null,
): PredictorInputs | null {
  if (!driverNumber) return null
  const driverLaps = laps
    .filter((lap) => Number(lap.driverNumber) === Number(driverNumber))
  ...
  const tyreAgeFromLaps = (() => {
    const latestCompound = normalizeCompound(latestLap.tyreCompound)
    if (!latestCompound) return null
    let age = 0
    for (let i = driverLaps.length - 1; i >= 0; i -= 1) {
      if (normalizeCompound(driverLaps[i].tyreCompound) !== latestCompound) break
      age += 1
    }
    return age > 0 ? age : null
  })()

  const currentCompound = normalizeCompound(
    currentStint?.tyre_compound || latestLap.tyreCompound || null
  )
  const pitLapSuggestion = firstNumber(
    currentStint?.optimal_pit_window,
    latestLap.pitOutSeconds != null ? latestLap.lapNumber + 1 : null,
    latestLap.lapNumber
  )

  return {
    position_before_pit: firstNumber(latestLap.position),
    tyre_age: firstNumber(currentStint?.tyre_age_at_stint_end, tyreAgeFromLaps),
    stint_length: firstNumber(currentStint?.tyre_laps_in_stint, tyreAgeFromLaps),
    compound: currentCompound,
    track_temp: null,
    pit_lap: pitLapSuggestion,
    race_name: raceName
  }
}
...
export const UndercutPredictor = React.memo(function UndercutPredictor() {
  ...
  const primaryDriverNumber = useMemo(() => {
    if (!primaryDriver) return null
    const driver = (sessionData?.drivers ?? []).find((item) => item.code === primaryDriver)
    return driver ? Number(driver.driverNumber) : null
  }, [primaryDriver, sessionData?.drivers])
  const laps = useMemo(() => (lapsStore.length ? lapsStore : sessionData?.laps ?? []), [lapsStore, sessionData?.laps])
  ...
  const sessionSeedInputs = useMemo(
    () => deriveInputsFromSession(laps, tyreStints, primaryDriverNumber, selectedRace ?? null),
    [laps, tyreStints, primaryDriverNumber, selectedRace]
  )
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((payload) => {
        ...
        const fromApi = deriveInputs(payload, selectedRace)
        const fromSession = sessionSeedInputs
        setInputs({
          ...fromApi,
          position_before_pit: fromApi.position_before_pit ?? fromSession?.position_before_pit ?? null,
          tyre_age: fromApi.tyre_age ?? fromSession?.tyre_age ?? null,
          stint_length: fromApi.stint_length ?? fromSession?.stint_length ?? null,
          compound: fromApi.compound ?? fromSession?.compound ?? null,
          pit_lap: fromApi.pit_lap ?? fromSession?.pit_lap ?? null,
          race_name: fromApi.race_name ?? fromSession?.race_name ?? selectedRace ?? null,
        })
      })
      .catch((err) => {
        if (cancelled) return
        ...
        setInputs(sessionSeedInputs ?? deriveInputs(null, selectedRace))
      })
      .finally(() => {
  ...
  }, [selectedYear, selectedRace, selectedSession, primaryDriverQuery, sessionSeedInputs])

  useEffect(() => {
    if (!sessionSeedInputs) return
    setInputs((prev) => ({
      ...prev,
      position_before_pit: prev.position_before_pit ?? sessionSeedInputs.position_before_pit,
      tyre_age: prev.tyre_age ?? sessionSeedInputs.tyre_age,
      stint_length: prev.stint_length ?? sessionSeedInputs.stint_length,
      compound: prev.compound ?? sessionSeedInputs.compound,
      pit_lap: prev.pit_lap ?? sessionSeedInputs.pit_lap,
      race_name: prev.race_name ?? sessionSeedInputs.race_name,
    }))
  }, [sessionSeedInputs])
  ...
  const onResetFromData = async () => {
    setReloadLoading(true)
    try {
      const apiInputs = deriveInputs(driverSummary, selectedRace ?? null)
      const localInputs = sessionSeedInputs
      setInputs({
        ...apiInputs,
        position_before_pit: localInputs?.position_before_pit ?? apiInputs.position_before_pit ?? null,
        tyre_age: localInputs?.tyre_age ?? apiInputs.tyre_age ?? null,
        stint_length: localInputs?.stint_length ?? apiInputs.stint_length ?? null,
        compound: localInputs?.compound ?? apiInputs.compound ?? null,
        pit_lap: localInputs?.pit_lap ?? apiInputs.pit_lap ?? null,
        race_name: selectedRace ?? localInputs?.race_name ?? apiInputs.race_name ?? null,
      })
      setError(null)
      setResult(null)
    } finally {
  ...
  const sourceLoading = summaryLoading || reloadLoading
      ...
      <div className="border-b border-border px-3 py-2.5">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px]">
          ...
          <span className="rounded border border-border bg-bg-secondary px-1.5 py-0.5 text-text-muted">
            {sourceLoading ? 'Source loading' : (driverSummary || sessionSeedInputs) ? 'Source synced' : 'Manual input mode'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-auto border-t border-border p-3">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          ...
          <span className="text-[11px] text-text-muted">
            {sourceLoading ? 'Loading feature data...' : 'Inputs seeded from session stints + /features/.../driver-summary'}
          </span>
        </div>
        ...
        {result && (
          <div className="rounded border border-border bg-bg-secondary p-2 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              <div className="font-mono text-sm text-text-primary">{RESULT_LABELS[result.prediction] ?? result.prediction}</div>
              <span className="rounded border border-border bg-bg-card px-1.5 py-0.5 font-mono text-[10px] text-text-primary">
                Success probability: {(result.success_probability * 100).toFixed(1)}%
              </span>
            </div>
