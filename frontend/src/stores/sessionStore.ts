...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  fetchSeasons: async () => {
    ...
    set((s) => ({ loadingState: s.sessionMeta ? 'ready' : 'loading', error: null, apiError: null }))
    try {
  ...
  fetchRaces: async (year) => {
    ...
    set((s) => ({
      loadingState: s.sessionMeta ? 'ready' : 'loading',
      error: null,
      apiError: null,
      selectedYear: year,
      selectedRace: null,
  ...
  loadSession: async (year, race, session) => {
    ...
    try {
      ...
      globalThis.setTimeout(() => {
        void fetchLaps(year, race, session).then(fullLaps => {
          if (requestId !== latestSessionRequestId) return
          const cur = get()
      ...
      globalThis.setTimeout(() => {
        void fetchPositions(year, race, session).then(posRows => {
          if (requestId !== latestSessionRequestId) return
          const cur = get()
          if (cur.selectedYear !== year || cur.selectedRace !== race || cur.selectedSession !== session) return
      ...
      globalThis.setTimeout(() => {
        void fetchSessionContext(year, race, session).then((ctx) => {
          if (requestId !== latestSessionRequestId) return
          const cur = get()
          if (cur.selectedYear !== year || cur.selectedRace !== race || cur.selectedSession !== session) return
          if (!cur.sessionData) return
          const weather = Array.isArray(ctx?.weather) ? ctx.weather : cur.sessionData.weather
          const raceControl = Array.isArray(ctx?.raceControl) ? ctx.raceControl : cur.sessionData.raceControl
          set({
            sessionData: {
              ...cur.sessionData,
              weather,
