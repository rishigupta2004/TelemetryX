...
export const ProfilesView = React.memo(function ProfilesView() {
  ...
  const sessionImages = useMemo(() => {
    ...
    for (const driver of sessionData?.drivers ?? []) {
      ...
      if (driver.teamColor) teamColorByName.set(driver.teamName, driver.teamColor)
      if (driver.code) driverCodeByName.set(driver.driverName, driver.code)
    }
    return { byDriver, byTeam, teamColorByName, driverCodeByName }
  }, [sessionData?.drivers])
              ...
              ? filteredDrivers.map((driver) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderLeftWidth: '3px',
                        borderLeftColor: sessionImages.teamColorByName.get(driver.teamName) || 'rgba(255,255,255,0.2)'
                      }}
                    >
                      {img ? (
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-[10px] text-fg-muted">
                          {(sessionImages.driverCodeByName.get(driver.driverName) || '').toUpperCase()} {driver.teamName}
                        </div>
                      </div>
                    </button>
                  )
              ...
              : filteredTeams.map((team) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderLeftWidth: '3px',
                        borderLeftColor: sessionImages.teamColorByName.get(team.teamName) || 'rgba(255,255,255,0.2)'
                      }}
                    >
                      {img ? (
          ...
          {mode === 'drivers' ? (
                ...
                <div className="flex flex-wrap items-start gap-3 rounded-md border border-border bg-bg-panel p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-lg font-semibold text-fg-primary">
                      {(sessionImages.driverCodeByName.get(selectedDriver.driverName) || selectedDriver.driverName.slice(0, 3).toUpperCase())}
                      {' '}
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-start gap-3 rounded-md border border-border bg-bg-panel p-3">
                ...
                <div className="min-w-0 flex-1">
                  <div
                    className="inline-flex rounded px-2 py-1 text-lg font-semibold text-fg-primary"
                    style={{
                      border: `1px solid ${sessionImages.teamColorByName.get(selectedTeam.teamName) || '#374151'}`,
