...
export const StandingsView = React.memo(function StandingsView() {
  ...
  const sessionDriverMeta = useMemo(() => {
    const byDriver = new Map<string, { code: string; teamColor: string; teamImage: string | null; driverImage: string | null }>()
    const byTeam = new Map<string, { teamColor: string; teamImage: string | null }>()
    for (const driver of sessionData?.drivers ?? []) {
      ...
      byDriver.set(driver.driverName, meta)
      byTeam.set(driver.teamName, { teamColor: meta.teamColor, teamImage: meta.teamImage })
    }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-2 p-2">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="bg-bg-surface border border-border rounded-md min-h-0 overflow-hidden">
          <div className="border-b border-border px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-fg-secondary">Driver Standings</div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 font-semibold">
                        <button type="button" className="flex items-center gap-2 text-left hover:text-accent-blue" onClick={() => openDriverProfile(driver.driverName)}>
                          <span className="inline-flex h-5 w-5 items-center justify-center rounded border border-white/10 bg-black/30 text-[9px] font-bold text-fg-secondary">
                            {sessionDriverMeta.byDriver.get(driver.driverName)?.code || driver.driverName.slice(0, 2).toUpperCase()}
                          </span>
                      ...
                      <td className="px-3 py-2 text-fg-secondary">
                        <div
                          ...
                          style={{
                            border: `1px solid ${sessionDriverMeta.byDriver.get(driver.driverName)?.teamColor ?? '#374151'}`,
        ...
        <div className="grid min-h-0 grid-rows-[0.95fr_1.05fr] gap-3">
          <div className="bg-bg-surface border border-border rounded-md min-h-0 overflow-hidden">
            <div className="border-b border-border px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-fg-secondary">Constructor Standings</div>
            <div className="max-h-full overflow-auto">
              <table className="w-full border-collapse text-left text-sm">
                <thead className="sticky top-0 bg-bg-surface text-[11px] uppercase tracking-[0.12em] text-fg-muted">
                  <tr>
                ...
                <tbody>
                  {payload.constructors.map((team) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 font-semibold">
                        <button type="button" className="flex items-center gap-2 text-left hover:text-accent-blue" onClick={() => openTeamProfile(team.teamName)}>
                          {sessionDriverMeta.byTeam.get(team.teamName)?.teamImage ? (
                            <img
                              src={sessionDriverMeta.byTeam.get(team.teamName)?.teamImage || ''}
                              alt={team.teamName}
                            ...
                            <span className="inline-flex h-5 w-5 items-center justify-center rounded border border-white/10 bg-black/30 text-[9px] font-bold text-fg-secondary">
                              {team.teamName.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase()}
                            </span>
                          )}
                          <span
                            className="rounded px-1.5 py-0.5"
                            style={{
                              border: `1px solid ${sessionDriverMeta.byTeam.get(team.teamName)?.teamColor ?? '#374151'}`,
                              backgroundColor: 'rgba(255,255,255,0.02)'
                            }}
                          >
                            {team.teamName}
          ...
          <div className="bg-bg-surface border border-border rounded-md min-h-0 px-4 py-3">
            ...
            <div className="mt-2 space-y-2 overflow-auto pr-1 text-xs">
              {topProgress.map((driver) => (
                <div key={`${driver.driverName}-${driver.driverNumber ?? 'x'}-progress`} className="rounded-md border border-border bg-bg-panel px-2.5 py-2">
                  <div className="mb-1 flex items-center justify-between">
                    <span className="font-semibold text-fg-primary">{sessionDriverMeta.byDriver.get(driver.driverName)?.code || driver.driverName}</span>
                    <span className="font-mono text-fg-secondary">{driver.points} pts</span>
                  </div>
                  <div className="grid grid-cols-6 gap-1">
                    {driver.seasonPointsProgression.slice(-6).map((race) => (
                      <div key={`${driver.driverName}-${race.raceName}`} className="rounded border border-border bg-white/5 px-1 py-1 text-center">
