...
function buildIndex(drivers: DriverLite[], allLaps: LapLite[], allPositions: PositionLite[]): DriverIndex[] {
  ...
  for (const row of allPositions) {
    ...
    if (!Number.isFinite(driverNumber) || !Number.isFinite(timestamp) || !Number.isFinite(x) || !Number.isFinite(y)) continue
    const bucket = posByNum.get(driverNumber) ?? []
    bucket.push({ ...row, driverNumber, timestamp, x, y })
    posByNum.set(driverNumber, bucket)
  }
  for (const rows of posByNum.values()) rows.sort((a, b) => a.timestamp - b.timestamp)

  for (const lap of allLaps) {
    const driverNumber = Number(lap.driverNumber)
    if (!Number.isFinite(driverNumber)) continue
    const normalizedLap = { ...lap, driverNumber }
    const byNum = lapsByNum.get(driverNumber) ?? []
    byNum.push(normalizedLap)
    lapsByNum.set(driverNumber, byNum)

    const driverCode = String(lap.driverName || '').toUpperCase()
    const byCode = lapsByCode.get(driverCode) ?? []
    byCode.push(normalizedLap)
    lapsByCode.set(driverCode, byCode)
  }
  ...
  return drivers.map((driver) => {
    ...
    const driverCode = String(driver.code || '').toUpperCase()
    const laps = lapsByNum.get(driverNumber) ?? lapsByCode.get(driverCode) ?? []
    const positions = posByNum.get(driverNumber) ?? []
...
function computePositions(sessionTime: number): CarPosition[] {
  ...
  for (const item of indexed) {
    ...
    const positionOffset = Number.isFinite(lapPosition) && lapPosition > 0 ? (lapPosition - 1) / 20 : 0
    const progressWithFallbackSpread = normalizeProgress(progress + positionOffset * 0.06)
    ...
    if (positions.length) {
      ...
      if (x != null && y != null && Math.abs(x) < 1e-9 && Math.abs(y) < 1e-9) {
    ...
    pos.progress = hasFreshPosition ? progress : progressWithFallbackSpread
    pos.currentLap = lap.lapNumber
    pos.position = Number.isFinite(lapPosition) && lapPosition > 0 ? lapPosition : 99
    pos.x = x
    pos.y = y
