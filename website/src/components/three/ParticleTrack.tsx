...
function TrackParticles({ count = 8000 }) {
  ...
  const [positions, phases, scatter, colors] = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Assign colors based on speed zones (simulated by random chunks)
      const rand = Math.random();
      if(rand > 0.9) {
        c[i * 3] = 1; c[i * 3 + 1] = 0.16; c[i * 3 + 2] = 0.16; // Red (braking)
      } else if (rand > 0.7) {
        c[i * 3] = 0.69; c[i * 3 + 1] = 0.15; c[i * 3 + 2] = 1; // Purple (fastest)
      } else {
        c[i * 3] = 0; c[i * 3 + 1] = 0.9; c[i * 3 + 2] = 1; // Cyan (normal)
      }
    }
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      phases[i] += delta * 0.15 * (1 + scatter[i * 3 + 1]); 
